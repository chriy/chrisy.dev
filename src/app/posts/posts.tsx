...
import { Calendar, GitBranch, MapPin, Search, Tag as TagIcon, Terminal } from "lucide-react";
        ...
        <section className="container-root mx-auto px-6 md:px-12 py-16 min-h-screen selection:bg-blue-500/20 relative">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 lg:gap-8">
                ...
                <div className="lg:col-span-8 relative">
                    ...
                    <div className="absolute left-10 top-0 bottom-0 hidden md:block">
                    ...
                    {filteredPosts.length === 0 && (
                        <div className="py-40 text-center ml-20">
                            <Terminal size={32} className="mx-auto mb-6 text-zinc-300 dark:text-zinc-600" />
...
const PostItem = ({ post, index }: { post: Post; index: number }) => {
    ...
    return (
        ...
        >
            ...
            <Link href={ `/posts/${ post.slug }` } className="block relative py-5 px-2 md:px-8 rounded-2xl transition-all duration-500 bg-transparent hover:bg-zinc-100/50 dark:hover:bg-zinc-800/10 group/card overflow-hidden">
                <article className="relative z-10 flex flex-col gap-4">
                    {/* Meta Row */}
                    <div className="flex items-center flex-wrap gap-x-5 gap-y-2">
                        <div className={ clsx("text-xs font-bold text-zinc-400 dark:text-zinc-500 tracking-widest flex items-center gap-2 uppercase", maple.className) }>
                        ...
                        <div className={clsx(
                            "text-xs flex items-center gap-1.5 font-bold text-zinc-300 dark:text-zinc-600 tracking-widest ml-auto",
                            maple.className
                        )}>
                    ...
                    <div className="space-y-3">
                        <h3 className={clsx(
                            "text-xl md:text-2xl font-bold tracking-tight text-neutral-800 dark:text-zinc-100 leading-snug group-hover/card:translate-x-1 transition-transform duration-500",
                            nunito.className
                        )}>
                            {post.metadata.title}
                        </h3>
        ...
        <div className="bg-zinc-50/30 dark:bg-[#050505]/40 border border-zinc-200/50 dark:border-white/5 p-6 rounded-3xl space-y-6 relative overflow-hidden group/widget">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                    ...
                    <h4 className={clsx("text-xs font-black uppercase tracking-widest text-zinc-900 dark:text-zinc-100", nunito.className)}>
                        Cluster Tags
                    </h4>
                </div>
                <div className="flex gap-3 items-center">
                    {allTags.length > 12 && (
                        <button
                            ...
                            className={clsx("text-xs font-bold text-zinc-400 hover:text-blue-400 transition-colors uppercase tracking-widest", nunito.className)}
                        >
                ...
                <div className="flex gap-3 items-center relative z-10">
                    ...
                    {selectedYear && (
                        <button
                            ...
                            className={clsx("text-xs font-bold text-blue-400 hover:underline uppercase tracking-wider", nunito.className)}
                        >
                            clear
                        </button>
                    )}
